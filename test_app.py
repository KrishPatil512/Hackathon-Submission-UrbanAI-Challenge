...
class EnvironmentConfigTests(unittest.TestCase):
    def test_env_file_loads_tomtom_alias_without_overwriting_shell_values(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            tomtom_value = "test-tomtom-value"
            file_secret_value = "test-file-value"
            shell_secret_value = "test-shell-value"
            env_file.write_text(
                f"TOMTOM_API_KEY={tomtom_value}\n"
                f"SAFEWALKERS_SECRET_KEY={file_secret_value}\n",
                encoding="utf-8",
            )

            with patch.dict(
                os.environ,
                {"SAFEWALKERS_SECRET_KEY": shell_secret_value},
                clear=True,
            ):
                load_env_file(env_file)

                self.assertEqual(os.environ["TT_API_KEY"], tomtom_value)
                self.assertEqual(os.environ["TOMTOM_API_KEY"], tomtom_value)
                self.assertEqual(os.environ["SAFEWALKERS_SECRET_KEY"], shell_secret_value)
